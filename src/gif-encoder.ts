 ...
 * @memberof GIFEncoder
 */
function generateImageDatas(frames: Frame[]) {
    ...
    otherFrams.forEach((frame) => {
        ...
        for (let index = 0; index < frame.pixels.length; index += 4) {
            const offset =
                ((Math.floor(index / 4 / w) + y) * frame.width +
                    x +
                    ((index / 4) % w)) *
                4;
            ...
            if ((r1 === r2 && g1 === g2 && b1 === b2) || a === 0) {
                ...
                maxStartOffset =
                    startOffset < maxStartOffset ? startOffset : maxStartOffset;
                endNum = 0;
            }
...
workPool.registerWork('generateImageDatas', generateImageDatas);
export default class GIFEncoder {
    private frames: Frame[] = [];

    public codes: number[] = [];

    addFrame(frame: IFrame | Frame) {
    ...
    async generatePalette() {
        ...
        // generateImageDatas(this.frames);
        ...
        if (this.palette.length) {
            ...
            this.colorDepth = Math.min(this.colorDepth, maxColorDepth);
        } else {
            this.colorDepth = 0;
        }
    }
    ...
    parseFramePixels(frame: Frame) {
        ...
        if (localColorMap.size + (hasTransparenc ? 1 : 0) > 256) {
            const nq = new NeuQuant(rgbPixels, {
        ...
        } else if (colorMap.size + (this.hasTransparenc ? 1 : 0) > 256) {
            //全局颜色板不够放,放到局部
            frame.isGlobalPalette = false;
            ...
            frames.map(async frame => {
                ...
                if (!frame.isGlobalPalette) {
                    const sizeOfColorTable =
                        Math.ceil(Math.log2(frame.palette.length / 3)) - 1;
